...
Aes.addRoundKey = (state, w, rnd, Nb) => {
    for (let r = 0; r < 4; r++) {
        for (let c = 0; c < Nb; c++) {
 ...
 */
Aes.Ctr.decrypt = (ciphertext, password, nBits) => {
    const blockSize = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!(128 === nBits || 192 === nBits || 256 === nBits)) {
        return ''; // standard allows 128/192/256 bit keys
    }
 ...
 * @param {Boolean} [utf8encode=false] Flag to indicate whether str is Unicode string to be encoded to UTF8 before conversion to base64; otherwise string is assumed to be 8-bit characters
 ...
 * @param {Boolean} [utf8decode=false] Flag to indicate whether str is Unicode string to be decoded from UTF8 after conversion from base64
...
Utf8.decode = (strUtf) => {
    ...
    let strUni = strUtf.replace(/[\u00e0-\u00ef][\u0080-\u00bf][\u0080-\u00bf]/g, // 3-byte chars
    (c) => {
